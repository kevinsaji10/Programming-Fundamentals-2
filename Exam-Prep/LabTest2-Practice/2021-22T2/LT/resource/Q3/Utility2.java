...
public class Utility2 {
    public static Map<String, List<University>> getListOfPartnerExchangeUniversities(List<Course> courseList) {
        Map<String, List<University>> map = new HashMap<>();
        for (Course c : courseList) {
            if (c instanceof ExchangeCourse) {
                ExchangeCourse ec = (ExchangeCourse) c;
                University u = ec.getUniversity();
                String country = u.getCountry();
                map.putIfAbsent(country, new ArrayList<University>());
                if (!map.get(country).contains(u)) {
                    map.get(country).add(u);
                }
            }
        }
        return map;
