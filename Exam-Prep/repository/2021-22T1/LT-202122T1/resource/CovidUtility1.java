...
public class CovidUtility1 {
    public static SimpleDate[] getInOfficeDates(String target, List<Entry> entries) throws IllegalArgumentException {
        if (entries == null || entries.size() == 0) {
            throw  new IllegalArgumentException("entries is null/empty");
        }
        List<SimpleDate> list = new ArrayList<>();
        for (Entry e : entries) {
            SimpleDate start = e.getStartDateTime().getDate();
            if (!list.contains(start) && e.getEmployeeId().equals(target)) {
                list.add(start);
            }

        }
        Collections.sort(list);
        return list.toArray(new SimpleDate[list.size()]);
    }
